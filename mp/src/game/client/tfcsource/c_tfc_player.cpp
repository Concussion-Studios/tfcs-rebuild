...
ConVar tfc_max_separation_force( "tfc_max_separation_force", "256", FCVAR_CHEAT | FCVAR_HIDDEN );
...
void __MsgFunc_ReloadEffect( bf_read &msg )
{
	...
	C_TFCPlayer *pPlayer = dynamic_cast< C_TFCPlayer* >( C_BaseEntity::Instance( iPlayer ) );
	if ( pPlayer )
...
BEGIN_RECV_TABLE_NOBASE( C_TFCPlayer, DT_TFCLocalPlayerExclusive )
...
BEGIN_RECV_TABLE_NOBASE( C_TFCPlayer, DT_TFCNonLocalPlayerExclusive )
	RecvPropVector( RECVINFO_NAME( m_vecNetworkOrigin, m_vecOrigin ) ),
...
IMPLEMENT_CLIENTCLASS_DT( C_TFCPlayer, DT_TFCPlayer, CTFCPlayer )
	...
	RecvPropDataTable( "tfc_localdata", 0, 0, &REFERENCE_RECV_TABLE(DT_TFCLocalPlayerExclusive) ),
	RecvPropDataTable( "tfc_nonlocaldata", 0, 0, &REFERENCE_RECV_TABLE(DT_TFCNonLocalPlayerExclusive) ),
...
LINK_ENTITY_TO_CLASS( player, C_TFCPlayer );

class C_TFCRagdoll : public C_BaseAnimatingOverlay
{
public:
	DECLARE_CLASS( C_TFCRagdoll, C_BaseAnimatingOverlay );
	DECLARE_CLIENTCLASS();
...
private:
	...
	C_TFCRagdoll( const C_TFCRagdoll & ) {}

	void Interp_Copy( C_BaseAnimatingOverlay *pDestinationEntity );
	void CreateTFCRagdoll( void );
...
IMPLEMENT_CLIENTCLASS_DT_NOBASE( C_TFCRagdoll, DT_TFCRagdoll, CTFCRagdoll )
...
void C_TFCRagdoll::Interp_Copy( C_BaseAnimatingOverlay *pSourceEntity )
{
	if ( !pSourceEntity )
		return;
...
void C_TFCRagdoll::ImpactTrace( trace_t *pTrace, int iDamageType, const char *pCustomImpactName )
{
...
void C_TFCRagdoll::CreateTFCRagdoll( void )
{
...
void C_TFCRagdoll::OnDataChanged( DataUpdateType_t type )
{
...
IRagdoll* C_TFCRagdoll::GetIRagdoll() const
{
	return m_pRagdoll;
}

void C_TFCRagdoll::UpdateOnRemove( void )
{
...
//-----------------------------------------------------------------------------
void C_TFCRagdoll::SetupWeights( const matrix3x4_t *pBoneToWorld, int nFlexWeightCount, float *pFlexWeights, float *pFlexDelayedWeights )
{
...
void C_TFCPlayer::ItemPreFrame( void )
{
...
void C_TFCPlayer::ItemPostFrame( void )
{
	if ( GetFlags() & FL_FROZEN )
		 return;
...
C_BaseAnimating *C_TFCPlayer::BecomeRagdollOnClient()
{
	// Let the C_TFCRagdoll entity do this.
	return NULL;
}

IRagdoll* C_TFCPlayer::GetRepresentativeRagdoll() const
{
	if ( m_hRagdoll.Get() )
	{
		C_TFCRagdoll *pRagdoll = (C_TFCRagdoll*)m_hRagdoll.Get();
...
const QAngle &C_TFCPlayer::EyeAngles()
{
	if( IsLocalPlayer() )
...
void C_TFCPlayer::AddEntity( void )
{
	BaseClass::AddEntity();
...
void C_TFCPlayer::ReleaseFlashlight( void )
{
	if( m_pFlashlightBeam )
	{
...
//-----------------------------------------------------------------------------
void C_TFCPlayer::UpdateFlashlight()
{
	// The dim light is the flashlight.
	if ( IsEffectActive( EF_DIMLIGHT ) )
	{
		if ( !m_pTFCFlashLightEffect )
		{
			// Turned on the headlight; create it.
			m_pTFCFlashLightEffect = new CTFCFlashlightEffect(index);

			if (!m_pTFCFlashLightEffect)
				return;

			m_pTFCFlashLightEffect->TurnOn();
		}
		...
		m_pTFCFlashLightEffect->UpdateLight( position, vecForward, vecRight, vecUp, FLASHLIGHT_DISTANCE );
	}
	else if ( m_pTFCFlashLightEffect )
	{
		// Turned off the flashlight; delete it.
		delete m_pTFCFlashLightEffect;
		m_pTFCFlashLightEffect = NULL;
	}
}

void CTFCFlashlightEffect::UpdateLight( const Vector &vecPos, const Vector &vecDir, const Vector &vecRight, const Vector &vecUp, int nDistance )
{
...
//=========================================================
Vector C_TFCPlayer::GetAutoaimVector( float flDelta )
{
...
// Purpose:
//-----------------------------------------------------------------------------
void C_TFCPlayer::ThirdPersonSwitch( bool bThirdPerson )
{
...
// Purpose:
//-----------------------------------------------------------------------------
void C_TFCPlayer::TeamChange( int iNewTeam )
{
...
// Purpose:
//-----------------------------------------------------------------------------
void C_TFCPlayer::TeamChangeStatic( int iNewTeam )
{
...
//-----------------------------------------------------------------------------
ShadowType_t C_TFCPlayer::ShadowCastType()
{
...
//-----------------------------------------------------------------------------
bool C_TFCPlayer::ShouldReceiveProjectedTextures( int flags )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
CStudioHdr *C_TFCPlayer::OnNewModel( void )
{
...
//-----------------------------------------------------------------------------
void C_TFCPlayer::InitializePoseParams( void )
{
	CStudioHdr *hdr = GetModelPtr();
	Assert( hdr );
	if ( !hdr )
...
void C_TFCPlayer::DoImpactEffect( trace_t &tr, int nDamageType )
{
	if ( GetActiveWeapon() )
	{
...
void C_TFCPlayer::LocalPlayerRespawn( void )
{
...
void C_TFCPlayer::OnDataChanged( DataUpdateType_t type )
{
	BaseClass::OnDataChanged( type );
...
void C_TFCPlayer::PlayReloadEffect()
{
	...
	CWeaponTFCBase *pWeapon = GetActiveTFCWeapon();
	if ( !pWeapon )
		return;

	// The weapon needs two models, world and view, but can only cache one. Synthesize the other.
	const CTFCWeaponInfo &info = pWeapon->GetTFCWpnData();
...
// Purpose: 
//-----------------------------------------------------------------------------
void C_TFCPlayer::CalculateIKLocks( float currentTime )
{
	if (!m_pIk) 
		return;
...
bool C_TFCPlayer::ShouldDraw( void )
{
...
void C_TFCPlayer::NotifyShouldTransmit( ShouldTransmitState_t state )
{
...
CWeaponTFCBase* C_TFCPlayer::GetActiveTFCWeapon() const
{
...
//-----------------------------------------------------------------------------
void C_TFCPlayer::CalcVehicleView(IClientVehicle* pVehicle, Vector& eyeOrigin, QAngle& eyeAngles, float& zNear, float& zFar, float& fov)
{
...
void C_TFCPlayer::CalcPlayerView(Vector& eyeOrigin, QAngle& eyeAngles, float& fov)
{
...
void C_TFCPlayer::CalcViewRoll( QAngle& eyeAngles )
{
...
void C_TFCPlayer::CalcViewBob( Vector& eyeOrigin )
{
	if ( !IsAlive() )
		return;
...
void C_TFCPlayer::CalcViewIdle(QAngle& eyeAngles)
{
	if ( !IsAlive() )
		return;
...
bool C_TFCPlayer::CanShowClassMenu( void )
{
	return ( GetTeamNumber() == TEAM_BLUE || 
			 GetTeamNumber() == TEAM_RED /*|| 
			 GetTeamNumber() == TEAM_GREEN || 
			 GetTeamNumber() == TEAM_YELLOW*/ );
}

bool C_TFCPlayer::CanShowTeamMenu( void )
{
	return true;
}

void C_TFCPlayer::ClientThink()
{
	UpdateSoundEvents();
...
// Purpose:
//-----------------------------------------------------------------------------
void C_TFCPlayer::UpdateLookAt( void )
{
	// head yaw
...
int C_TFCPlayer::GetIDTarget() const
{
...
//-----------------------------------------------------------------------------
void C_TFCPlayer::UpdateIDTarget()
{
	if ( !IsLocalPlayer() )
		return;
...
//-----------------------------------------------------------------------------
void C_TFCPlayer::AvoidPlayers( CUserCmd *pCmd )
{
	// Don't test if the player doesn't exist or is dead.
	if ( IsAlive() == false )
		return;

	C_TFCTeam *pTeam = ( C_TFCTeam * )GetTeam();
	if ( !pTeam )
		return;
	...
	Vector vecTFCPlayerCenter = GetAbsOrigin();
	Vector vecTFCPlayerMin = GetPlayerMins();
	Vector vecTFCPlayerMax = GetPlayerMaxs();
	float flZHeight = vecTFCPlayerMax.z - vecTFCPlayerMin.z;
	vecTFCPlayerCenter.z += 0.5f * flZHeight;
	VectorAdd( vecTFCPlayerMin, vecTFCPlayerCenter, vecTFCPlayerMin );
	VectorAdd( vecTFCPlayerMax, vecTFCPlayerCenter, vecTFCPlayerMax );

	// Find an intersecting player or object.
	int nAvoidPlayerCount = 0;
	C_TFCPlayer *pAvoidPlayerList[MAX_PLAYERS];

	C_TFCPlayer *pIntersectPlayer = NULL;
	float flAvoidRadius = 0.0f;

	Vector vecAvoidCenter, vecAvoidMin, vecAvoidMax;
	for ( int i = 0; i < pTeam->GetNumPlayers(); ++i )
	{
		C_TFCPlayer *pAvoidPlayer = static_cast< C_TFCPlayer * >( pTeam->GetPlayer( i ) );
		...
		if ( IsBoxIntersectingBox( vecTFCPlayerMin, vecTFCPlayerMax, vecAvoidMin, vecAvoidMax ) )
		{
	...
	if ( pIntersectPlayer )
	{
		VectorSubtract( pIntersectPlayer->WorldSpaceCenter(), vecTFCPlayerCenter, vecDelta );
	...
	float flPushStrength = RemapValClamped( vecDelta.Length(), flAvoidRadius, 0, 0, tfc_max_separation_force.GetInt() ); //flPushScale;
...
bool C_TFCPlayer::CreateMove( float flInputSampleTime, CUserCmd *pCmd )
{	
...
void C_TFCPlayer::UpdateSoundEvents()
{
	int iNext;
	for ( int i=m_SoundEvents.Head(); i != m_SoundEvents.InvalidIndex(); i = iNext )
	{
		...
		CTFCSoundEvent *pEvent = &m_SoundEvents[i];
	...
	{
		C_TFCPlayer *pPlayer = ToTFCPlayer( C_TFCPlayer::GetLocalTFCPlayer() );
