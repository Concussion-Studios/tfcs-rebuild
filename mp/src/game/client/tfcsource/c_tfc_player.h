...
#define TFC_AVOID_MAX_RADIUS_SQR		5184.0f			// Based on player extents and max buildable extents.
#define TFC_OO_AVOID_MAX_RADIUS_SQR		0.00029f
#define FLASHLIGHT_DISTANCE 1000

//Tony; m_pFlashlightEffect is private, so just subclass. We may want to do some more stuff with it later anyway.
class CTFCFlashlightEffect : public CFlashlightEffect
{
public:
	CTFCFlashlightEffect(int nIndex = 0) : CFlashlightEffect( nIndex  ) {}
	~CTFCFlashlightEffect() {};
...
private:
	...
	CTFCFlashlightEffect *m_pTFCFlashLightEffect;

	int m_ArmorValue, m_MaxArmorValue;

	class CTFCSoundEvent
	{
	public:
		string_t m_SoundName;
		float m_flEventTime;	// Play the event when gpGlobals->curtime goes past this.
	};
	CUtlLinkedList<CTFCSoundEvent,int> m_SoundEvents;

};


inline C_TFCPlayer* ToTFCPlayer( CBaseEntity *pPlayer )
{
...
inline TFCPlayerState C_TFCPlayer::State_Get() const
{
