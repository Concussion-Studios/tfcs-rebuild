...
float ClientModeTFCNormal::GetViewModelFOV( void )
{
	...
	auto *pWeapon = (C_WeaponTFCBase*)GetActiveWeapon();
	if ( pWeapon )
...
// Purpose: 
//-----------------------------------------------------------------------------
bool ClientModeTFCNormal::DoPostScreenSpaceEffects( const CViewSetup *pSetup )
{
#ifdef GLOWS_ENABLE
...
//-----------------------------------------------------------------------------
void ClientModeTFCNormal::OverrideView( CViewSetup *pSetup )
{
	QAngle camAngles;
...
// Purpose: 
//-----------------------------------------------------------------------------
void ClientModeTFCNormal::FireGameEvent( IGameEvent *event )
{
	if ( Q_strcmp( "player_team", event->GetName() ) == 0 )
	{
		...
		if ( pPlayer && pPlayer->IsLocalPlayer() )
			...
			C_TFCPlayer::TeamChangeStatic( team ); // that's me.
...
// Purpose:
//-----------------------------------------------------------------------------
bool ClientModeTFCNormal::CanRecordDemo( char *errorMsg, int length ) const
{
	auto *player = C_TFCPlayer::GetLocalTFCPlayer();
	if ( !player )
		return true;
...
//-----------------------------------------------------------------------------
int	ClientModeTFCNormal::KeyInput( int down, ButtonCode_t keynum, const char *pszCurrentBinding )
{
	// If we're voting...
