...
{
	auto *pPlayer = C_TFCPlayer::GetLocalTFCPlayer();
	if ( pPlayer == NULL )
		return;

	CWeaponTFCBase *pWeapon = pPlayer->GetActiveTFCWeapon();		
	if( !pWeapon )
		return;
	...
	if( pPlayer->m_Shared.IsSniperZoomed() &&
		pWeapon->GetTFCWpnData().m_bUseScope &&
		pWeapon->GetTFCWpnData().m_bIsSniper )
	{
		int screenWide, screenTall;
		GetHudSize(screenWide, screenTall);
