...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CTFCScoreboard::CTFCScoreboard( IViewPort *pViewPort ) : CClientScoreBoardDialog( pViewPort )
{
	m_iImageDead = 0;
...
//-----------------------------------------------------------------------------
void CTFCScoreboard::PaintBackground()
{
	int wide, tall;
	GetSize( wide, tall );
...
//-----------------------------------------------------------------------------
void CTFCScoreboard::PaintBorder()
{
	int wide, tall;
	GetSize( wide, tall );
...
//-----------------------------------------------------------------------------
void CTFCScoreboard::ApplySchemeSettings( vgui::IScheme *pScheme )
{
...
//-----------------------------------------------------------------------------
bool CTFCScoreboard::TFCPlayerSortFunc( vgui::SectionedListPanel *list, int itemID1, int itemID2 )
{
...
//-----------------------------------------------------------------------------
void CTFCScoreboard::InitPlayerList( SectionedListPanel *pPlayerList, int teamNumber )
{
	pPlayerList->SetVerticalScrollbar( false );
	pPlayerList->RemoveAll();
	pPlayerList->RemoveAllSections();
	pPlayerList->AddSection( 0, "Players", TFCPlayerSortFunc );
...
//-----------------------------------------------------------------------------
void CTFCScoreboard::UpdateTeamInfo()
{
	// update the team sections in the scoreboard
	int startTeam = TEAM_UNASSIGNED;

	if ( TFCGameRules()->IsTeamplay() )
		startTeam = TEAM_BLUE;
...
//-----------------------------------------------------------------------------
void CTFCScoreboard::UpdatePlayerList()
{
	m_pPlayerListDM->RemoveAll();
	m_pPlayerListRed->RemoveAll();
	m_pPlayerListBlue->RemoveAll();

	C_TFCPlayer *pLocalPlayer = C_TFCPlayer::GetLocalTFCPlayer();
	if ( !pLocalPlayer )
		return;
...
//-----------------------------------------------------------------------------
void CTFCScoreboard::UpdateSpectatorList()
{
	C_TFCPlayer *pLocalPlayer = C_TFCPlayer::GetLocalTFCPlayer();
	if ( !pLocalPlayer )
		return;
...
//-----------------------------------------------------------------------------
bool CTFCScoreboard::ShouldShowAsSpectator( int iPlayerIndex )
{
	...
	if ( g_PR->IsConnected( iPlayerIndex ) ) 
	{
		...
		if ( TFCGameRules()->IsTeamplay() && TEAM_UNASSIGNED == iTeam )
			return true;
...
//-----------------------------------------------------------------------------
void CTFCScoreboard::FireGameEvent( IGameEvent *event )
{
...
//-----------------------------------------------------------------------------
bool CTFCScoreboard::GetPlayerScoreInfo( int playerIndex, KeyValues *kv )
{
	C_TFC_PlayerResource *sdk_PR = dynamic_cast<C_TFC_PlayerResource *>( g_PR );
	if ( !sdk_PR )
		return true;
	...
	if( ( localteam == TEAM_SPECTATOR || localteam == TEAM_UNASSIGNED || team == localteam ) )
	{
		...
		if( g_PR->IsConnected( playerIndex ) )
		{
			C_TFCTeam *pTeam = dynamic_cast<C_TFCTeam *>( GetGlobalTeam( team ) );
			...
			{
				const CTFCPlayerClassInfo &info = pTeam->GetPlayerClassInfo( cls );
...
void CTFCScoreboard::UpdateItemVisibiity()
{
