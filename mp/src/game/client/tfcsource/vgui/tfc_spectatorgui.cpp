...
DECLARE_HUDELEMENT( CTFCMapOverview )
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CTFCSpectatorGUI::CTFCSpectatorGUI(IViewPort *pViewPort) : CSpectatorGUI(pViewPort)
{
	m_pBlueLabel =		NULL;
	m_pBlueScore =		NULL;
	m_pRedLabel =		NULL;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CTFCSpectatorGUI::ApplySchemeSettings(vgui::IScheme *pScheme)
{
...
//-----------------------------------------------------------------------------
void CTFCSpectatorGUI::UpdateSpectatorPlayerList()
{
	C_TFCTeam *blue = GetGlobalTFCTeam( TEAM_BLUE );
	if ( blue )
	{
		wchar_t frags[ 10 ];
	...
	C_TFCTeam *red = GetGlobalTFCTeam( TEAM_RED );
	if ( red )
	{
		wchar_t frags[ 10 ];
	...
	/*C_TFCTeam *green = GetGlobalTFCTeam( TEAM_GREEN );
	if ( green )
	{
		wchar_t frags[ 10 ];
	...
	C_TFCTeam *yellow = GetGlobalTFCTeam( TEAM_YELLOW );
	if ( yellow )
	{
		wchar_t frags[ 10 ];
...
bool CTFCSpectatorGUI::NeedsUpdate( void )
{
	C_TFCPlayer *player = C_TFCPlayer::GetLocalTFCPlayer();
	if ( !player )
		return false;

	if ( m_nLastTime != (int)TFCGameRules()->GetMapRemainingTime() )
		return true;
...
//-----------------------------------------------------------------------------
void CTFCSpectatorGUI::UpdateTimer()
{
	wchar_t szText[ 63 ];

	
	m_nLastTime = (int)( TFCGameRules()->GetMapRemainingTime() );
...
//-----------------------------------------------------------------------------
void CTFCSpectatorGUI::StoreWidths( void )
{
	if ( !ControlsPresent() )
		return;
...
//-----------------------------------------------------------------------------
void CTFCSpectatorGUI::ResizeControls( void )
{
	if ( !ControlsPresent() )
		return;
...
//-----------------------------------------------------------------------------
bool CTFCSpectatorGUI::ControlsPresent( void ) const
{
...
void CTFCMapOverview::InitTeamColorsAndIcons()
{
...
//-----------------------------------------------------------------------------
void CTFCMapOverview::ApplySchemeSettings(vgui::IScheme *scheme)
{
...
//-----------------------------------------------------------------------------
void CTFCMapOverview::Update( void )
{
...
//-----------------------------------------------------------------------------
CTFCMapOverview::TFCMapPlayer_t* CTFCMapOverview::GetTFCInfoForPlayerIndex( int index )
{
	if ( index < 0 || index >= MAX_PLAYERS )
		return NULL;

	return &m_PlayersTFCInfo[ index ];
}

//-----------------------------------------------------------------------------
CTFCMapOverview::TFCMapPlayer_t* CTFCMapOverview::GetTFCInfoForPlayer(MapPlayer_t *player)
{
	if( player == NULL )
		return NULL;
...
bool CTFCMapOverview::CanPlayerBeSeen( MapPlayer_t *player )
{
	...
	TFCMapPlayer_t *tfcPlayer = GetTFCInfoForPlayer(player);
	...
	if( player->health <= 0 )
	{
		...
		if ( tfcPlayer->overrideExpirationTime == -1  ||  tfcPlayer->overrideExpirationTime <= gpGlobals->curtime )
			return false;
	}
...
CTFCMapOverview::CTFCMapOverview( const char *pElementName ) : BaseClass( pElementName )
{
...
void CTFCMapOverview::Init( void )
{
	BaseClass::Init();
}

CTFCMapOverview::~CTFCMapOverview()
{
	g_pMapOverview = NULL;
...
void CTFCMapOverview::UpdatePlayers()
{
	C_TFC_PlayerResource *pTFCPR = (C_TFC_PlayerResource*)GameResources();
	if ( !pTFCPR )
		return;
	...
	for ( int i = 1; i<= gpGlobals->maxClients; i++)
	{
		...
		TFCMapPlayer_t *playerTFC = GetTFCInfoForPlayerIndex(i-1);

		if ( !playerTFC )
			continue;

		// update from global player resources
		if ( pTFCPR->IsConnected(i) )
		{
			player->health = pTFCPR->GetHealth( i );

			if ( !pTFCPR->IsAlive( i ) )
			{
				// Safety actually happens after a TKPunish.
				player->health = 0;
				playerTFC->isDead = true;
			}

			if ( player->team != pTFCPR->GetTeam( i ) )
			{
				player->team = pTFCPR->GetTeam( i );
...
bool CTFCMapOverview::ShouldDraw( void )
{
...
CTFCMapOverview::MapPlayer_t* CTFCMapOverview::GetPlayerByEntityID( int entityID )
{
...
bool CTFCMapOverview::AdjustPointToPanel(Vector2D *pos)
{
	if( pos == NULL )
		return false;
...
void CTFCMapOverview::DrawMapTexture()
{
	int alpha = GetMasterAlpha();
...
bool CTFCMapOverview::DrawIconTFC( int textureID, int offscreenTextureID, Vector pos, float scale, float angle, int alpha, bool allowRotation, const char *text, Color *textColor, float status, Color *statusColor )
{
	if( alpha <= 0 )
		return false;
...
void CTFCMapOverview::DrawMapPlayers()
{
	...
	for (int i=0; i < MAX_PLAYERS; i++)
	{
		int alpha = 255;
		MapPlayer_t *player = &m_Players[i];
		TFCMapPlayer_t *playerTFC = GetTFCInfoForPlayerIndex(i);
		...
		// Now draw them
		if( playerTFC->overrideExpirationTime > gpGlobals->curtime )// If dead, an X, if alive, an alpha'd normal icon
		{
			int alphaToUse = alpha;
			if( playerTFC->overrideFadeTime != -1 && playerTFC->overrideFadeTime <= gpGlobals->curtime )
			{
				// Fade linearly from fade start to disappear
				alphaToUse *= 1 - (float)(gpGlobals->curtime - playerTFC->overrideFadeTime) / (float)(playerTFC->overrideExpirationTime - playerTFC->overrideFadeTime);
			}

			DrawIconTFC( playerTFC->overrideIcon, playerTFC->overrideIconOffscreen, playerTFC->overridePosition, m_flIconSize * 1.1f, GetViewAngle(), player->health > 0 ? alphaToUse / 2 : alphaToUse, true, name, &player->color, -1, &colorGreen );
			if( player->health > 0 )
				DrawIconTFC( m_playerFacing, -1, playerTFC->overridePosition, m_flIconSize * 1.1f, playerTFC->overrideAngle[YAW], player->health > 0 ? alphaToUse / 2 : alphaToUse, true, name, &player->color, status, &colorGreen );
		}
		else
		{
			float zDifference = 0;
			...
			DrawIconTFC( normalIcon, offscreenIcon, player->position, sizeForPlayer, angleForPlayer, alpha, true, name, &player->color, status, &colorGreen );
			if( !doingLocalPlayer )
			{
				// Draw the facing for everyone but the local player.
				if( player->health > 0 )
					DrawIconTFC( m_playerFacing, -1, player->position, sizeForPlayer, player->angle[YAW], alpha, true, name, &player->color, status, &colorGreen );
			}
		}
	}
}

void CTFCMapOverview::SetMap(const char * levelname)
{
...
void CTFCMapOverview::ResetRound()
{
	BaseClass::ResetRound();

	for (int i=0; i<MAX_PLAYERS; i++)
	{
		TFCMapPlayer_t *p = &m_PlayersTFCInfo[i];
...
void CTFCMapOverview::DrawCamera()
{
	C_BasePlayer *localPlayer = C_BasePlayer::GetLocalPlayer();
	...
	if( localPlayer->GetObserverMode() == OBS_MODE_ROAMING )
	{
		...
		int alpha = 255;
		DrawIconTFC(m_cameraIconFree, m_cameraIconFree, localPlayer->GetAbsOrigin(), m_flIconSize * 3.0f, localPlayer->EyeAngles()[YAW], alpha);
	}
	else if( localPlayer->GetObserverMode() == OBS_MODE_IN_EYE )
	{
		if( localPlayer->GetObserverTarget() )
		{
			...
			int alpha = 255 * 0.5f;
			DrawIconTFC(m_cameraIconFirst, m_cameraIconFirst, localPlayer->GetObserverTarget()->GetAbsOrigin(), m_flIconSize * 1.5f, GetViewAngle(), alpha);
		}
	}
	else if( localPlayer->GetObserverMode() == OBS_MODE_CHASE )
	{
		if( localPlayer->GetObserverTarget() )
		{
			...
			int alpha = 255;
			DrawIconTFC(m_cameraIconThird, m_cameraIconThird, localPlayer->GetObserverTarget()->GetAbsOrigin(), m_flIconSize * 3.0f, localPlayer->EyeAngles()[YAW], alpha);
		}
	}
}

void CTFCMapOverview::FireGameEvent( IGameEvent *event )
{
	...
	if ( Q_strcmp(type,"player_death") == 0 )
	{
		...
		TFCMapPlayer_t *playerTFC = GetTFCInfoForPlayer(player);

		if ( !playerTFC )
			return;

		playerTFC->isDead = true;
		playerTFC->overrideIcon = m_TeamIconsDead[GetIconNumberFromTeamNumber(player->team)];
		playerTFC->overrideIconOffscreen = playerTFC->overrideIcon;
		playerTFC->overridePosition = player->position;
		playerTFC->overrideAngle = player->angle;
		playerTFC->overrideFadeTime = gpGlobals->curtime + DEATH_ICON_FADE;
		playerTFC->overrideExpirationTime = gpGlobals->curtime + DEATH_ICON_DURATION;
	}
...
void CTFCMapOverview::SetMode(int mode)
{
...
void CTFCMapOverview::UpdateSizeAndPosition()
{
	int x,y,w,h;
...
void CTFCMapOverview::SetPlayerSeen( int index )
{
	TFCMapPlayer_t *pCS = GetTFCInfoForPlayerIndex(index);
...
//-----------------------------------------------------------------------------
void CTFCMapOverview::SetPlayerPreferredMode( int mode )
{
...
//-----------------------------------------------------------------------------
void CTFCMapOverview::SetPlayerPreferredViewSize( float viewSize )
{
...
//-----------------------------------------------------------------------------
int CTFCMapOverview::GetIconNumberFromTeamNumber( int teamNumber )
{
	switch(teamNumber) 
	{
	case TEAM_BLUE:
...
//-----------------------------------------------------------------------------
int CTFCMapOverview::GetMasterAlpha( void )
{
...
//-----------------------------------------------------------------------------
int CTFCMapOverview::GetBorderSize( void )
{
	switch( GetMode() )
	{
		case MAP_MODE_INSET:
...
//-----------------------------------------------------------------------------
Vector2D CTFCMapOverview::PanelToMap( const Vector2D &panelPos )
{
