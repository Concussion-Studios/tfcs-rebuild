...
class CTFCBot : public CTFCPlayer
{
public:
	bool			m_bBackwards;
...
LINK_ENTITY_TO_CLASS( tfc_bot, CTFCBot );
...
public:
	static CBasePlayer* ClientPutInServerOverride_Bot( edict_t *pEdict, const char *playername )
	{
		...
		CTFCBot *pPlayer = static_cast<CTFCBot *>( CreateEntityByName( "tfc_bot" ) );
		if ( pPlayer )
		{
...
CBasePlayer *BotPutInServer( bool bFrozen )
{
	...
	// This trick lets us create a CTFCBot for this client instead of the CTFCPlayer
	...
	CTFCBot *pPlayer = ((CTFCBot*)CBaseEntity::Instance( pEdict ));
...
void Puppet_RunAll( void )
{
	for ( int i = 1; i <= gpGlobals->maxClients; i++ )
	{
		...
		{
			CTFCBot *pBot = dynamic_cast< CTFCBot* >( pPlayer );
			if ( pBot )
				Bot_Think( pBot );
		}
	}
...
// Output : 	virtual void
//-----------------------------------------------------------------------------
static void RunPlayerMove( CTFCPlayer *fakeclient, CUserCmd &cmd, float frametime )
{
	if ( !fakeclient )
		return;
...
void Bot_UpdateStrafing( CTFCBot *pBot, CUserCmd &cmd )
{
...
void Bot_UpdateDirection( CTFCBot *pBot )
{
...
void Bot_FlipOut( CTFCBot *pBot, CUserCmd &cmd )
{
...
void Bot_HandleSendCmd( CTFCBot *pBot )
{
...
void Bot_ForceFireWeapon( CTFCBot *pBot, CUserCmd &cmd )
{
...
void Bot_SetForwardMovement( CTFCBot *pBot, CUserCmd &cmd )
{
...
void Bot_HandleRespawn( CTFCBot *pBot, CUserCmd &cmd )
{
