...
void FinishClientPutInServer( CTFCPlayer *pPlayer )
{
	...
	//Tony; changed from old TFC, we want to start out dead etc beacuse we're using states.
...
void ClientGamePrecache( void )
{
	...
	for ( KeyValues *pData = pValues->GetFirstSubKey(); pData != NULL; pData = pData->GetNextKey() )
	{
			 ...
			 Q_strlen( pszFile ) > 0 )
		{
			...
			ConColorMsg( Color( 86, 156, 143, 255 ), "[CTFCGameRules] Precahing %s is now ready to use.\n", pszFile );
		}
	}
