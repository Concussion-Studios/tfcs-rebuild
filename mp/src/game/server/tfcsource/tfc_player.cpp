...
#include "weapon_basetfcgrenade.h"
...
ConVar TFC_ShowStateTransitions("tfc_ShowStateTransitions", "-2", FCVAR_CHEAT, "tfc_ShowStateTransitions <ent index or -1 for all>. Show player state transitions.");
...
public:
	...
	{
		auto *pPlayer = (CTFCPlayer*)pObject->GetGameData();
		if (pPlayer)
		{
...
// Tables.
// -------------------------------------------------------------------------------- //
BEGIN_DATADESC(CTFCPlayer)
DEFINE_THINKFUNC(TFCPushawayThink),
...
BEGIN_SEND_TABLE_NOBASE(CTFCPlayer, DT_TFCLocalPlayerExclusive)
...
BEGIN_SEND_TABLE_NOBASE(CTFCPlayer, DT_TFCNonLocalPlayerExclusive)
...
// Data that only gets sent to the local player.
SendPropDataTable("tfc_localdata", 0, &REFERENCE_SEND_TABLE(DT_TFCLocalPlayerExclusive), SendProxy_SendLocalDataTable),
// Data that gets sent to all other players
SendPropDataTable("tfc_nonlocaldata", 0, &REFERENCE_SEND_TABLE(DT_TFCNonLocalPlayerExclusive), SendProxy_SendNonLocalDataTable),
...
// Ragdoll entities.
// -------------------------------------------------------------------------------- //
class CTFCRagdoll : public CBaseAnimatingOverlay
{
public:
	DECLARE_CLASS(CTFCRagdoll, CBaseAnimatingOverlay);
	DECLARE_SERVERCLASS();
...
LINK_ENTITY_TO_CLASS(tfc_ragdoll, CTFCRagdoll);

IMPLEMENT_SERVERCLASS_ST_NOBASE(CTFCRagdoll, DT_TFCRagdoll)
...
void CTFCPlayer::SetupVisibility(CBaseEntity *pViewEntity, unsigned char *pvs, int pvssize)
{
...
CTFCPlayer::CTFCPlayer()
{
	...
	m_PlayerAnimState = CreateTFCPlayerAnimState(this);
...
void CTFCPlayer::LeaveVehicle(const Vector &vecExitPoint, const QAngle &vecExitAngles)
{
...
void CTFCPlayer::TFCPushawayThink(void)
{
	// Push physics props out of our way.
	PerformObstaclePushaway(this);
	SetNextThink(gpGlobals->curtime + PUSHAWAY_THINK_INTERVAL, TFC_PUSHAWAY_THINK_CONTEXT);
}
...
void CTFCPlayer::Spawn()
{
	...
	m_impactEnergyScale = TFC_PHYSDAMAGE_SCALE;
	...
	SetContextThink(&CTFCPlayer::TFCPushawayThink, gpGlobals->curtime + PUSHAWAY_THINK_INTERVAL, TFC_PUSHAWAY_THINK_CONTEXT);
	pl.deadflag = false;

}

bool CTFCPlayer::SelectSpawnSpot(const char *pEntClassName, CBaseEntity* &pSpot)
{
	...
	DevMsg("CTFCPlayer::SelectSpawnSpot: couldn't find valid spawn point.\n");
...
//=========================================================
//=========================================================
void CTFCPlayer::CheatImpulseCommands(int iImpulse)
{
...
// Give ammo
//=========================================================
int CTFCPlayer::GiveAmmo(int iCount, int iAmmoIndex, bool bSuppressSound)
{
	int team = GetTeamNumber();
	if (team != TEAM_SPECTATOR)
	{
		CTFCTeam *pTeam = GetGlobalTFCTeam(team);
		const CTFCPlayerClassInfo &TFCInfo = pTeam->GetPlayerClassInfo(m_Shared.PlayerClass());

		int iMaxAmmo = TFCInfo.m_aMaxAmmo[iAmmoIndex];
...
// Discard unused ammo
//=========================================================
void CTFCPlayer::DiscardAmmo(bool bDead)
{
...
{
	CTFCPlayer *pPlayer = dynamic_cast<CTFCPlayer *>(UTIL_GetCommandClient());
	if (pPlayer)
	{
		pPlayer->DiscardAmmo();
	}
...
//-----------------------------------------------------------------------------
bool CTFCPlayer::BumpWeapon(CBaseCombatWeapon *pWeapon)
{
...
void CTFCPlayer::ChangeTeam(int iTeamNum)
{
	...
	{
		Warning("CTFCPlayer::ChangeTeam( %d ) - invalid team index.\n", iTeamNum);
		return;
	}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CTFCPlayer::CommitSuicide(bool bExplode /* = false */, bool bForce /*= false*/)
{
	...
	m_iSuicideCustomKillFlags = TFC_DMG_CUSTOM_SUICIDE;
...
int CTFCPlayer::OnTakeDamage(const CTakeDamageInfo &inputInfo)
{
...
int CTFCPlayer::OnTakeDamage_Alive(const CTakeDamageInfo &info)
{
	...
	if (!(m_Shared.DesiredPlayerClass() == PLAYERCLASS_HEAVY))
	{
			...
			(!attacker->IsSolidFlagSet(FSOLID_TRIGGER)))
		{
			...
			CWeaponTFCBase *pWeapon = (CWeaponTFCBase *)GetActiveWeapon();
			if (pWeapon)
			{
				CBaseTFCGrenade *pGrenade = dynamic_cast<CBaseTFCGrenade *>(pWeapon);
				if (pGrenade)
...
void CTFCPlayer::OnDamagedByExplosion(const CTakeDamageInfo &info)
{
...
void CTFCPlayer::BecomeAGibs(const CTakeDamageInfo &info)
{
...
Class_T CTFCPlayer::Classify(void)
{
	if (IsInAVehicle())
	{
		IServerVehicle *pVehicle = GetVehicle();
...
void CTFCPlayer::ClearDamagerHistory()
{
	for (int i = 0; i < ARRAYSIZE(m_DamagerHistory); i++)
		m_DamagerHistory[i].Reset();
}

void CTFCPlayer::AddDamagerToHistory(EHANDLE hDamager)
{
	//Check if same team
	CTFCPlayer *pDamager = ToTFCPlayer(hDamager);
...
// TraceAttack
//=========================================================
void CTFCPlayer::TraceAttack(const CTakeDamageInfo &inputInfo, const Vector &vecDir, trace_t *ptr, CDmgAccumulator *pAccumulator)
{
	if (m_takedamage != DAMAGE_YES)
		return;

	CTFCPlayer *pAttacker = (CTFCPlayer*)ToTFCPlayer(inputInfo.GetAttacker());
	if (pAttacker)
	{
...
void CTFCPlayer::InitVCollision(const Vector &vecAbsOrigin, const Vector &vecAbsVelocity)
{
	BaseClass::InitVCollision(vecAbsOrigin, vecAbsVelocity);

	// Setup the TFC specific callback.
	IPhysicsPlayerController *pPlayerController = GetPhysicsController();
	if (pPlayerController)
		pPlayerController->SetEventHandler(&playerCallback);
}

void CTFCPlayer::FireBullets(const FireBulletsInfo_t &info)
{
	...
	CWeaponTFCBase *pWeapon = dynamic_cast<CWeaponTFCBase *>(GetActiveWeapon());
	if (pWeapon)
		modinfo.m_iPlayerDamage = modinfo.m_flDamage = pWeapon->GetTFCWpnData().m_iDamage;
...
void CTFCPlayer::CreateRagdollEntity()
{
	if (m_hRagdoll)
	{
	...
	CTFCRagdoll *pRagdoll = dynamic_cast< CTFCRagdoll* >(m_hRagdoll.Get());

	if (!pRagdoll)
	{
		// create a new one
		pRagdoll = dynamic_cast< CTFCRagdoll* >(CreateEntityByName("tfc_ragdoll"));
	}
...
//-----------------------------------------------------------------------------
void CTFCPlayer::DestroyRagdoll(void)
{
	CTFCRagdoll *pRagdoll = dynamic_cast<CTFCRagdoll*>(m_hRagdoll.Get());
	if (pRagdoll)
	{
...
//			the TFC animation state to setup the hitboxes.
//-----------------------------------------------------------------------------
void CTFCPlayer::SetupBones(matrix3x4_t *pBoneToWorld, int boneMask)
{
	VPROF_BUDGET("CTFCPlayer::SetupBones", VPROF_BUDGETGROUP_SERVER_ANIM);
...
CWeaponTFCBase* CTFCPlayer::GetActiveTFCWeapon() const
{
	return dynamic_cast< CWeaponTFCBase* >(GetActiveWeapon());
}

void CTFCPlayer::SetHandsModel( const char* model )
{
	if ( !model || !(*model) ) 
		return;
...
void CTFCPlayer::CreateViewModel( int index /*=0*/ )
{
...
void CTFCPlayer::FlashlightTurnOn(void)
{
...
void CTFCPlayer::FlashlightTurnOff(void)
{
...
int CTFCPlayer::FlashlightIsOn(void)
{
...
bool CTFCPlayer::ClientCommand(const CCommand &args)
{
	...
	{
		CTFCTeam *pTeam = GetGlobalTFCTeam(GetTeamNumber());
...
bool CTFCPlayer::HandleCommand_JoinTeam(int team)
{
	CTFCGameRules *mp = TFCGameRules();
	int iOldTeam = GetTeamNumber();
	...
	if (mp->TeamStacked(team, GetTeamNumber()))//players are allowed to change to their own team so they can just change their model
	{
		...
		ClientPrint(
			this,
			HUD_PRINTCENTER,
			(team == TEAM_BLUE) ? "#BlueTeam_full" : "#RedTeam_full" ? "#YellowTeam_full" : "#GreenTeam_full");
...
bool CTFCPlayer::HandleCommand_JoinClass(int iClass)
{
	...
	if (!TFCGameRules()->IsPlayerClassOnTeam(iClass, GetTeamNumber()))
		return false;

	const char *classname = TFCGameRules()->GetPlayerClassName(iClass, GetTeamNumber());

	if (TFCGameRules()->CanPlayerJoinClass(this, iClass))
	{
...
void CTFCPlayer::ShowClassSelectMenu()
{
	if (GetTeamNumber() == TFC_TEAM_BLUE)
		ShowViewPortPanel(PANEL_CLASS_BLUE);
	else if (GetTeamNumber() == TFC_TEAM_RED)
		ShowViewPortPanel(PANEL_CLASS_RED);
	else if (GetTeamNumber() == TFC_TEAM_YELLOW)
		ShowViewPortPanel(PANEL_CLASS_YELLOW);
	else if (GetTeamNumber() == TFC_TEAM_GREEN)
		ShowViewPortPanel(PANEL_CLASS_GREEN);
}

void CTFCPlayer::SetClassMenuOpen(bool bOpen)
{
	m_bIsClassMenuOpen = bOpen;
}

bool CTFCPlayer::IsClassMenuOpen(void)
{
...
void CTFCPlayer::State_Enter(TFCPlayerState newState)
{
	...
	if (TFC_ShowStateTransitions.GetInt() == -1 || TFC_ShowStateTransitions.GetInt() == entindex())
	{
		if (m_pCurStateInfo)
...
void CTFCPlayer::State_PreThink()
{
...
CTFCPlayerStateInfo* CTFCPlayer::State_LookupInfo(TFCPlayerState state)
{
	// This table MUST match the 
	static CTFCPlayerStateInfo playerStateInfos[] =
	{
		{ STATE_ACTIVE, "STATE_ACTIVE", &CTFCPlayer::State_Enter_ACTIVE, NULL, &CTFCPlayer::State_PreThink_ACTIVE },
		{ STATE_WELCOME, "STATE_WELCOME", &CTFCPlayer::State_Enter_WELCOME, NULL, &CTFCPlayer::State_PreThink_WELCOME },
		{ STATE_PICKINGTEAM, "STATE_PICKINGTEAM", &CTFCPlayer::State_Enter_PICKINGTEAM, NULL, &CTFCPlayer::State_PreThink_WELCOME },
		{ STATE_PICKINGCLASS, "STATE_PICKINGCLASS", &CTFCPlayer::State_Enter_PICKINGCLASS, NULL, &CTFCPlayer::State_PreThink_WELCOME },
		{ STATE_DEATH_ANIM, "STATE_DEATH_ANIM", &CTFCPlayer::State_Enter_DEATH_ANIM, NULL, &CTFCPlayer::State_PreThink_DEATH_ANIM },
		{ STATE_OBSERVER_MODE, "STATE_OBSERVER_MODE", &CTFCPlayer::State_Enter_OBSERVER_MODE, NULL, &CTFCPlayer::State_PreThink_OBSERVER_MODE }
	};
...
void CTFCPlayer::MoveToNextIntroCamera()
{
...
void CTFCPlayer::State_PreThink_WELCOME()
{
...
void CTFCPlayer::State_Enter_DEATH_ANIM()
{
	if (HasWeapons())
	{
...
void CTFCPlayer::State_PreThink_DEATH_ANIM()
{
	...
	if (gpGlobals->curtime >= (m_flDeathTime + TFC_PLAYER_DEATH_TIME))	// let the death cam stay going up to min spawn time.
	{
...
void CTFCPlayer::State_PreThink_OBSERVER_MODE()
{
...
void CTFCPlayer::State_PreThink_ACTIVE()
{
}

int CTFCPlayer::GetPlayerStance()
{
...
void CTFCPlayer::NoteWeaponFired(void)
{
...
bool CTFCPlayer::WantsLagCompensationOnEntity(const CBasePlayer *pPlayer, const CUserCmd *pCmd, const CBitVec<MAX_EDICTS> *pEntityTransmitBits) const
{
...
void CTFCPlayer::IncrementHealthValue(int nCount)
{
	m_iHealth += nCount;
...
void CTFCPlayer::SetHealth(int value)
{
	m_iHealth = value;
}

void CTFCPlayer::SetMaxHealth(int MaxValue)
{
	m_iMaxHealth = MaxValue;
}

void CTFCPlayer::AdjustHealth(int HealthValue)
{
	m_iHealth = m_iMaxHealth = HealthValue;
}

void CTFCPlayer::IncrementArmorValue(int nCount, int nMaxValue)
{
...
void CTFCPlayer::SetArmorValue(int value)
{
	m_ArmorValue = value;
}

void CTFCPlayer::SetMaxArmorValue(int MaxArmorValue)
{
	m_MaxArmorValue = MaxArmorValue;
}

void CTFCPlayer::AdjustArmor(int ArmorValue)
{
