...
#define TFC_PUSHAWAY_THINK_CONTEXT	"TFCPushawayThink"
#define TFC_PHYSDAMAGE_SCALE 4.0f

// Function table for each player state.
class CTFCPlayerStateInfo
{
public:
	...
	void (CTFCPlayer::*pfnPreThink)();	// Do a PreThink() in this state.
};
...
// >> TFC Game player
//=============================================================================
class CTFCPlayer : public CBaseMultiplayerPlayer
{
public:
	DECLARE_CLASS( CTFCPlayer, CBaseMultiplayerPlayer );
	DECLARE_SERVERCLASS();
	DECLARE_PREDICTABLE();
	DECLARE_DATADESC();
	...
	CWeaponTFCBase* GetWeaponOwnerID( int iID );
	...
	//CTFCPlayerClassInfo* GetClassInfo(void);
...
public:
	...
	CNetworkVarEmbedded( CTFCPlayerShared, m_Shared );
	virtual void PlayerDeathThink( void ) {	/* overridden, do nothing - our states handle this now */ }
	virtual bool ClientCommand( const CCommand &args );

	Vector GetAttackSpread( CWeaponTFCBase *pWeapon, CBaseEntity *pTarget = NULL );
...
private:
	...
	void State_Enter( TFCPlayerState newState );	// Initialize the new state.
...
private:
	...
	static CTFCPlayerStateInfo* State_LookupInfo( TFCPlayerState state );
	...
	CTFCPlayerStateInfo *m_pCurStateInfo;			// This can be NULL if no state info is defined for m_iPlayerState.
...
private:
	...
	CTFCPlayerAnimState *m_PlayerAnimState;
...
inline TFCPlayerState CTFCPlayer::State_Get() const
{
