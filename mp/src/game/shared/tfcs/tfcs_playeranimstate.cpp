...
#define TFCS_RUN_SPEED				320.0f
#define TFCS_WALK_SPEED			75.0f
#define TFCS_CROUCHWALK_SPEED		110.0f
...
CTFCSPlayerAnimState* CreateTFCSPlayerAnimState( CTFCSPlayer *pPlayer )
{
	...
	movementData.m_flRunSpeed = TFCS_RUN_SPEED;
	movementData.m_flWalkSpeed = TFCS_WALK_SPEED;
	movementData.m_flSprintSpeed = -1.0f;

	// Create animation state for this player.
	CTFCSPlayerAnimState *pRet = new CTFCSPlayerAnimState( pPlayer, movementData );

	// Specific TFCS player initialization.
	pRet->InitTFCSAnimState( pPlayer );
...
CTFCSPlayerAnimState::CTFCSPlayerAnimState()
{
	...
	// Don't initialize TFCS specific variables here. Init them in InitTFCSAnimState()
}
...
//			&movementData - 
//-----------------------------------------------------------------------------
CTFCSPlayerAnimState::CTFCSPlayerAnimState( CBasePlayer *pPlayer, MultiPlayerMovementData_t &movementData )
...
//-----------------------------------------------------------------------------
// Purpose: Initialize TFCS specific animation state.
// Input  : *pPlayer - 
//-----------------------------------------------------------------------------
void CTFCSPlayerAnimState::InitTFCSAnimState( CTFCSPlayer *pPlayer )
{
	m_pTFCSPlayer = pPlayer;
}

//-----------------------------------------------------------------------------
// Purpose: 
//-----------------------------------------------------------------------------
void CTFCSPlayerAnimState::ClearAnimationState( void )
{
...
// Output : Activity
//-----------------------------------------------------------------------------
Activity CTFCSPlayerAnimState::TranslateActivity( Activity actDesired )
{
	// Hook into baseclass when / if tfcs player models get swim animations.
	Activity translateActivity = actDesired; //BaseClass::TranslateActivity( actDesired );

	if ( GetTFCSPlayer()->GetActiveWeapon() )
	{
		translateActivity = GetTFCSPlayer()->GetActiveWeapon()->ActivityOverride( translateActivity, false );
	}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CTFCSPlayerAnimState::Update( float eyeYaw, float eyePitch )
{
	// Profile the animation update.
	VPROF( "CTFCSPlayerAnimState::Update" );

	// Get the TFCS player.
	CTFCSPlayer *pTFCSPlayer = GetTFCSPlayer();
	if ( !pTFCSPlayer )
		return;

	// Get the studio header for the player.
	CStudioHdr *pStudioHdr = pTFCSPlayer->GetModelPtr();
	if ( !pStudioHdr )
		return;
...
void CTFCSPlayerAnimState::ComputePlaybackRate()
{
	...
	float maxspeed = m_pTFCSPlayer->GetSequenceGroundSpeed( m_pTFCSPlayer->GetSequence() );
	...
	if ( isMoving && ( maxspeed > 0.0f ) )
	{
		...
		m_pTFCSPlayer->SetPlaybackRate( ( speed * flFactor ) / maxspeed );
	...
	{
		m_pTFCSPlayer->SetPlaybackRate( 1.0f );
	}
}
...
void CTFCSPlayerAnimState::DoAnimationEvent( PlayerAnimEvent_t event, int nData )
{
	...
	case PLAYERANIMEVENT_ATTACK_PRIMARY:
		{
			// Weapon primary fire.
			if ( m_pTFCSPlayer->GetFlags() & FL_DUCKING )
	...
	{
		CBaseCombatWeapon *pWeapon = GetTFCSPlayer()->GetActiveWeapon();
		if ( pWeapon )
		{
...
// Input  : *idealActivity - 
//-----------------------------------------------------------------------------
bool CTFCSPlayerAnimState::HandleSwimming( Activity &idealActivity )
{
...
//-----------------------------------------------------------------------------
bool CTFCSPlayerAnimState::HandleMoving( Activity &idealActivity )
{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CTFCSPlayerAnimState::HandleDucking( Activity &idealActivity )
{
	if ( m_pTFCSPlayer->GetFlags() & FL_DUCKING )
	{
...
//-----------------------------------------------------------------------------
// Purpose: 
bool CTFCSPlayerAnimState::HandleJumping( Activity &idealActivity )
{
	Vector vecVelocity;
	...
	if ( m_bJumping )
	{
		static bool bNewJump = false; //Tony; tfcs players only have a 'hop'
		...
		if ( m_pTFCSPlayer->GetWaterLevel() >= WL_Waist )
		{
			m_bJumping = false;
			RestartMainSequence();
		}
		...
		{
			if ( m_pTFCSPlayer->GetFlags() & FL_ONGROUND )
			{
				m_bJumping = false;
				RestartMainSequence();
...
bool CTFCSPlayerAnimState::SetupPoseParameters( CStudioHdr *pStudioHdr )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CTFCSPlayerAnimState::EstimateYaw( void )
{
	// Get the frame time.
...
//-----------------------------------------------------------------------------
float SnapYawTo( float flValue );
void CTFCSPlayerAnimState::ComputePoseParam_MoveYaw( CStudioHdr *pStudioHdr )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CTFCSPlayerAnimState::ComputePoseParam_AimPitch( CStudioHdr *pStudioHdr )
{
	// Get the view pitch.
...
// Purpose: 
//-----------------------------------------------------------------------------
void CTFCSPlayerAnimState::ComputePoseParam_AimYaw( CStudioHdr *pStudioHdr )
{
...
//-----------------------------------------------------------------------------
// Purpose: Override the default, because tfcs models don't use moveX
// Input  :  - 
// Output : float
//-----------------------------------------------------------------------------
