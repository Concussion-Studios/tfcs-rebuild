...
ConVar	tfc_clamp_back_speed( "tfc_clamp_back_speed", "0.9", FCVAR_REPLICATED | FCVAR_CHEAT );
ConVar  tfc_clamp_back_speed_min( "tfc_clamp_back_speed_min", "100", FCVAR_REPLICATED | FCVAR_CHEAT );
...
public:
	...
	void SetTFCDuckedEyeOffset( float duckFraction );
...
void CTFCGameMovement::SetPlayerSpeed( void )
{
	...
	{
		mv->m_flClientMaxSpeed = m_pTFCPlayer->m_Shared.m_flMaxSpeed;	//gets cut in fraction later
	}
	else
	{
		flMaxSpeed = m_pTFCPlayer->m_Shared.m_flMaxSpeed;	//jogging
...
// Purpose: 
//-----------------------------------------------------------------------------
void CTFCGameMovement::CheckParameters( void )
{
	QAngle	v_angle;
...
bool CTFCGameMovement::CanAccelerate()
{
	...
	TFCPlayerState curState = m_pTFCPlayer->State_Get();
...
void CTFCGameMovement::WalkMove( void )
{
	...
	if ( tfc_clamp_back_speed.GetFloat() < 1.0 && VectorLength( mv->m_vecVelocity ) > tfc_clamp_back_speed_min.GetFloat() )
	{
		...
		if ( flDot < 0 )
		{
			...
			float flMaxBackSpeed = ( mv->m_flMaxSpeed * tfc_clamp_back_speed.GetFloat() );
...
void CTFCGameMovement::CategorizePosition( void )
{
	// Observer.
...
inline void CTFCGameMovement::TracePlayerBBoxWithStep( const Vector &vStart, const Vector &vEnd, 
							unsigned int fMask, int collisionGroup, trace_t &trace )
{
	VPROF( "CTFCGameMovement::TracePlayerBBoxWithStep" );
...
bool CTFCGameMovement::ResolveStanding( void )
{
	VPROF( "CTFCGameMovement::ResolveStanding" );
...
//-----------------------------------------------------------------------------
void CTFCGameMovement::LimitForwardVel(void)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CTFCGameMovement::CheckJumpButton( void )
{
	if (!m_pTFCPlayer->IsAlive())
	{
	...
	if ( m_pTFCPlayer->GetWaterLevel() >= 2 )
	{	
		// swimming, not jumping
		SetGroundEntity( NULL );

		if(m_pTFCPlayer->GetWaterType() == CONTENTS_WATER)    // We move up a certain amount
			mv->m_vecVelocity[2] = 100;
		else if (m_pTFCPlayer->GetWaterType() == CONTENTS_SLIME)
	...
	// No more effect
	if (m_pTFCPlayer->GetGroundEntity() == NULL)
	{
	...
	m_pTFCPlayer->PlayStepSound( (Vector &)mv->GetAbsOrigin(), player->GetSurfaceData(), 1.0, true );
	m_pTFCPlayer->DoAnimationEvent( PLAYERANIMEVENT_JUMP );

	// make the jump sound unless we aren't the spy
	if ( !( m_pTFCPlayer->m_Shared.DesiredPlayerClass() == PLAYERCLASS_HEAVY ) )
	{
		CPASFilter filter( m_pTFCPlayer->GetAbsOrigin() );
		filter.UsePredictionRules();
		m_pTFCPlayer->EmitSound( filter, m_pTFCPlayer->entindex(), "Player.Jump" );	
	}
	...
	if ( (  m_pTFCPlayer->m_Local.m_bDucking ) || (  m_pTFCPlayer->GetFlags() & FL_DUCKING ) )
	{
	...
	m_pTFCPlayer->m_Shared.SetJumping( true );
...
//-----------------------------------------------------------------------------
void CTFCGameMovement::FinishUnDuck( void )
{
	int i;
	trace_t trace;
	Vector newOrigin;
...
//-----------------------------------------------------------------------------
void CTFCGameMovement::FinishDuck( void )
{
	Vector hullSizeNormal = VEC_HULL_MAX - VEC_HULL_MIN;
	Vector hullSizeCrouch = VEC_DUCK_HULL_MAX - VEC_DUCK_HULL_MIN;
...
// Input  : duckFraction - 
//-----------------------------------------------------------------------------
void CTFCGameMovement::SetTFCDuckedEyeOffset( float duckFraction )
{
...
//-----------------------------------------------------------------------------
void CTFCGameMovement::Duck( void )
{
		...
		if ( mv->m_nButtons & IN_DUCK )
		{
			...
			if ( player->m_Local.m_bDucking )
			{
				...
				else
				{
					...
					SetTFCDuckedEyeOffset( flDuckFraction );
				}
			}
		}
		else
				...
				if ( CanUnduck() )
				{
						 ...
						 player->m_Local.m_bDucked ) // or unducking
					{
						...
						else
						{
							...
							SetTFCDuckedEyeOffset( duckFraction );
						}
					}
				}
				else
...
//-----------------------------------------------------------------------------
unsigned int CTFCGameMovement::PlayerSolidMask( bool brushOnly )
{
	unsigned int uMask = 0;

	if ( m_pTFCPlayer )
	{
		switch( m_pTFCPlayer->GetTeamNumber() )
		{
		case TFC_TEAM_RED:
...
// Output : const Vector
//-----------------------------------------------------------------------------
Vector CTFCGameMovement::GetPlayerMins( void ) const
{
	if ( !player )
	{
		return vec3_origin;
...
// Output : const Vector
//-----------------------------------------------------------------------------
Vector CTFCGameMovement::GetPlayerMaxs( void ) const
{	
	if ( !player )
	{
		return vec3_origin;
