...
#include "tfc_basegrenade_projectile.h"
...
static CTFCViewVectors g_TFCViewVectors(
...
const CViewVectors* CTFCGameRules::GetViewVectors() const
{
	return (CViewVectors*)GetTFCViewVectors();
}

const CTFCViewVectors *CTFCGameRules::GetTFCViewVectors() const
{
	return &g_TFCViewVectors;
}

// --------------------------------------------------------------------------------------------------- //
// CTFCGameRules implementation.
// --------------------------------------------------------------------------------------------------- //
CTFCGameRules::CTFCGameRules()
{
#ifndef CLIENT_DLL
	ConColorMsg(Color(86, 156, 143, 255), "[C_TFCGameRules] Creating Gamerules....\n");
	...
	if (filesystem->FileExists(UTIL_VarArgs("maps/cfg/%s.cfg", STRING(gpGlobals->mapname))))
	{
		...
		ConColorMsg(Color(86, 156, 143, 255), "[TFCGameRules] Executing map %s config file\n", STRING(gpGlobals->mapname));
		engine->ServerCommand(UTIL_VarArgs("exec %s.cfg */maps\n", STRING(gpGlobals->mapname)));
		engine->ServerExecute();
	}
	else
	{
		ConColorMsg(Color(86, 156, 143, 255), "[TFCGameRules] Could not load map %s config file skiping...\n", STRING(gpGlobals->mapname));
	}
#else
	ConColorMsg(Color(86, 156, 143, 255), "[TFCGameRules] Creating Gamerules....\n");
#endif
}
...
{
#ifndef CLIENT_DLL
	ConColorMsg(Color(86, 156, 143, 255), "[CTFCGameRules] Destroying Gamerules....\n");

	// Note, don't delete each team since they are in the gEntList and will 
	// automatically be deleted from there, instead.
	g_Teams.Purge();
#else
	ConColorMsg(Color(86, 156, 143, 255), "[C_TFCGameRules] Destroying Gamerules....\n");
#endif
}
...
//-----------------------------------------------------------------------------
bool CTFCGameRules::InGameMode(int nGamemode)
{
...
//-----------------------------------------------------------------------------
void CTFCGameRules::AddGameMode(int nGamemode)
{
	Assert(nGamemode >= 0 && nGamemode < GAMEMODE_LAST);
...
//-----------------------------------------------------------------------------
void CTFCGameRules::RemoveGameMode(int nGamemode)
{
	Assert(nGamemode >= 0 && nGamemode < GAMEMODE_LAST);
	m_nGamemode &= ~(1 << nGamemode);
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CTFCGameRules::CheckGameMode()
{
	ConColorMsg(Color(86, 156, 143, 255), "[TFCGameRules] Executing server global gamemode config file\n");
	...
	ConColorMsg( Color( 86, 156, 143, 255 ), "[TFCGameRules] Executing server CTF gamemode config file\n" );
	...
	ConColorMsg( Color( 86, 156, 143, 255 ), "[TFCGameRules] Executing server TC gamemode config file\n" );
	...
	ConColorMsg( Color( 86, 156, 143, 255 ), "[TFCGameRules] Executing server CP gamemode config file\n" );
	...
	ConColorMsg( Color( 86, 156, 143, 255 ), "[TFCGameRules] Executing server AD gamemode config file\n" );
	engine->ServerCommand( "exec config_default_cp.cfg \n" );
	engine->ServerExecute();
	}
	...
	if ((gEntList.FindEntityByClassname(NULL, "logic_tdm") || !Q_strncmp(STRING(gpGlobals->mapname), "tdm_", 4) || !Q_strncmp(STRING(gpGlobals->mapname), "dm_", 3)) || mp_4team.GetBool())
	{
		...
		ConColorMsg(Color(86, 156, 143, 255), "[TFCGameRules] Executing server TDM gamemode config file\n");
	...
	if (gEntList.FindEntityByClassname(NULL, "logic_esc") || !Q_strncmp(STRING(gpGlobals->mapname), "esc_", 4) || !Q_strncmp(STRING(gpGlobals->mapname), "vip_", 4))
	{
		...
		ConColorMsg(Color(86, 156, 143, 255), "[TFCGameRules] Executing server Escort/VIP gamemode config file\n");
	...
	if (gEntList.FindEntityByClassname(NULL, "logic_dom") || !Q_strncmp(STRING(gpGlobals->mapname), "dom_", 4))
	{
		...
		ConColorMsg(Color(86, 156, 143, 255), "[TFCGameRules] Executing server Domination gamemode config file\n");
		engine->ServerCommand("exec config_default_dom.cfg \n");
		engine->ServerExecute();
	}
}

void CTFCGameRules::ServerActivate()
{
	...
	{
		ConColorMsg(Color(255, 215, 0, 255), "[TFCGameRules] Trying to set a NaN game start time!\n");
		m_flGameStartTime.GetForModify() = 0.0f;
	}
}

void CTFCGameRules::CheckLevelInitialized()
{
...
void CTFCGameRules::ClientSettingsChanged(CBasePlayer *pPlayer)
{
	...
	CTFCPlayer *pTFCPlayer = (CTFCPlayer*)pPlayer;

	if (pTFCPlayer->IsFakeClient())
		return;

	const char *pszFov = engine->GetClientConVarValue(pPlayer->entindex(), "fov_desired");
	int iFov = atoi(pszFov);
	iFov = clamp(iFov, 75, MAX_FOV);
	pTFCPlayer->SetDefaultFOV(iFov);
}
...
CBaseEntity *CTFCGameRules::GetPlayerSpawnSpot(CBasePlayer *pPlayer)
{
...
bool CTFCGameRules::IsSpawnPointValid(CBaseEntity *pSpot, CBasePlayer *pPlayer)
{
...
void CTFCGameRules::PlayerSpawn(CBasePlayer *p)
{
	CTFCPlayer *pPlayer = ToTFCPlayer(p);
	...
	if (team != TEAM_SPECTATOR)
	{
		...
		{
			CTFCTeam *pTeam = GetGlobalTFCTeam(team);
			const CTFCPlayerClassInfo &pClassInfo = pTeam->GetPlayerClassInfo(playerclass);
...
void CTFCGameRules::ChooseRandomClass(CTFCPlayer *pPlayer)
{
	int i;
	int numChoices = 0;
	int choices[16];
	...
	CTFCTeam *pTeam = GetGlobalTFCTeam(pPlayer->GetTeamNumber());
	...
	if (numChoices == 0)
	{
		ConColorMsg(Color(255, 215, 0, 255), "[TFCGameRules] Random class found that all classes were full - ignoring class limits for this spawn!\n");
...
bool CTFCGameRules::CanPlayerJoinClass(CTFCPlayer *pPlayer, int cls)
{
...
bool CTFCGameRules::ReachedClassLimit(int team, int cls)
{
...
int CTFCGameRules::CountPlayerClass(int team, int cls)
{
	int num = 0;
	CTFCPlayer *pTFCPlayer;

	for (int i = 1; i <= gpGlobals->maxClients; i++)
	{
		pTFCPlayer = ToTFCPlayer(UTIL_PlayerByIndex(i));

		if (pTFCPlayer == NULL)
			continue;

		if (FNullEnt(pTFCPlayer->edict()))
			continue;

		if (pTFCPlayer->GetTeamNumber() != team)
			continue;

		if (pTFCPlayer->m_Shared.DesiredPlayerClass() == cls)
			num++;
	}
...
int CTFCGameRules::GetClassLimit(int team, int cls)
{
	CTFCTeam *pTeam = GetGlobalTFCTeam(team);

	Assert(pTeam);

	const CTFCPlayerClassInfo &pClassInfo = pTeam->GetPlayerClassInfo(cls);
...
bool CTFCGameRules::IsPlayerClassOnTeam(int cls, int team)
{
	if (cls == PLAYERCLASS_RANDOM)
		return true;
...
void CTFCGameRules::InitTeams(void)
{
	...
	CTeam *pUnassigned = static_cast<CTeam*>(CreateEntityByName("tfc_team_unassigned"));
	Assert(pUnassigned);
	pUnassigned->Init(pszTeamNames[TEAM_UNASSIGNED], TEAM_UNASSIGNED);
	g_Teams.AddToTail(pUnassigned);

	//Tony; just use a plain ole tfc_team_manager for spectators
	CTeam *pSpectator = static_cast<CTeam*>(CreateEntityByName("tfc_team_manager"));
	Assert(pSpectator);
	pSpectator->Init(pszTeamNames[TEAM_SPECTATOR], TEAM_SPECTATOR);
	g_Teams.AddToTail(pSpectator);

	//Tony; create the blue team
	CTeam *pBlue = static_cast<CTeam*>(CreateEntityByName("tfc_team_blue"));
	Assert(pBlue);
	pBlue->Init(pszTeamNames[TFC_TEAM_BLUE], TFC_TEAM_BLUE);
	g_Teams.AddToTail(pBlue);

	//Tony; create the red team
	CTeam *pRed = static_cast<CTeam*>(CreateEntityByName("tfc_team_red"));
	Assert(pRed);
	pRed->Init(pszTeamNames[TFC_TEAM_RED], TFC_TEAM_RED);
	g_Teams.AddToTail(pRed);

	//Anthony; create the yellow team
	CTeam *pYellow = static_cast<CTeam*>(CreateEntityByName("tfc_team_yellow"));
	Assert(pYellow);
	pYellow->Init(pszTeamNames[TFC_TEAM_YELLOW], TFC_TEAM_YELLOW);
	g_Teams.AddToTail(pYellow);

	//Anthony; create the greem team
	CTeam *pGreen = static_cast<CTeam*>(CreateEntityByName("tfc_team_green"));
	Assert(pGreen);
	pGreen->Init(pszTeamNames[TFC_TEAM_GREEN], TFC_TEAM_GREEN);
	g_Teams.AddToTail(pGreen);
}

/* create some proxy entities that we use for transmitting data */
void CTFCGameRules::CreateStandardEntities()
{
	// Create the player resource
	g_pPlayerResource = (CPlayerResource*)CBaseEntity::Create("tfc_player_manager", vec3_origin, vec3_angle);
...
int CTFCGameRules::SelectDefaultTeam()
{
	int team = TEAM_UNASSIGNED;

	CTFCTeam *pBlue = GetGlobalTFCTeam(TFC_TEAM_BLUE);
	CTFCTeam *pRed = GetGlobalTFCTeam(TFC_TEAM_RED);
	CTFCTeam *pYellow = GetGlobalTFCTeam(TFC_TEAM_YELLOW);
	CTFCTeam *pGreen = GetGlobalTFCTeam(TFC_TEAM_GREEN);
	...
	else
	{
		...
		if (TFCGameRules() && TFCGameRules()->IsTDMGamemode())
			team = (random->RandomInt(0, 3) == 0) ? TFC_TEAM_BLUE : TFC_TEAM_RED ? TFC_TEAM_YELLOW : TFC_TEAM_GREEN;
		else
			team = (random->RandomInt(0, 1) == 0) ? TFC_TEAM_BLUE : TFC_TEAM_RED;
	}

	if (TeamFull(team))
	{
		// Pick the opposite team
		if (team == TFC_TEAM_BLUE)
		{
			team = TFC_TEAM_RED;
		}
		else if (team == TFC_TEAM_RED)
		{
			team = TFC_TEAM_YELLOW;
		}
		else
...
bool CTFCGameRules::TeamFull(int team_id)
{
	...
	case TFC_TEAM_BLUE:
	{
		int iNumBlue = GetGlobalTFCTeam(TFC_TEAM_BLUE)->GetNumPlayers();
		return iNumBlue >= m_iSpawnPointCount_Blue;
	}
	case TFC_TEAM_RED:
	{
		int iNumRed = GetGlobalTFCTeam(TFC_TEAM_RED)->GetNumPlayers();
		return iNumRed >= m_iSpawnPointCount_Red;
	}
	case TFC_TEAM_YELLOW:
	{
		int iNumRed = GetGlobalTFCTeam(TFC_TEAM_YELLOW)->GetNumPlayers();
		return iNumRed >= m_iSpawnPointCount_Yellow;
	}
	case TFC_TEAM_GREEN:
	{
		int iNumRed = GetGlobalTFCTeam(TFC_TEAM_GREEN)->GetNumPlayers();
...
bool CTFCGameRules::TeamStacked(int iNewTeam, int iCurTeam)
{
	...
	int iNumBlue = GetGlobalTeam(TFC_TEAM_BLUE)->GetNumPlayers();
	int iNumRed = GetGlobalTeam(TFC_TEAM_RED)->GetNumPlayers();
	int iNumYellow = GetGlobalTeam(TFC_TEAM_YELLOW)->GetNumPlayers();
	int iNumGreen = GetGlobalTeam(TFC_TEAM_GREEN)->GetNumPlayers();
...
//-----------------------------------------------------------------------------
const char *CTFCGameRules::GetKillingWeaponName(const CTakeDamageInfo &info, CTFCPlayer *pVictim, int *iWeaponID)
{
	CBaseEntity *pInflictor = info.GetInflictor();
	CBaseEntity *pKiller = info.GetAttacker();
	CBasePlayer *pScorer = TFCGameRules()->GetDeathScorer(pKiller, pInflictor, pVictim);

	const char *killer_weapon_name = "world";
	*iWeaponID = TFC_WEAPON_NONE;
	...
	if (pScorer && pInflictor && (pInflictor == pScorer))
	{
		...
		if (pScorer->GetActiveWeapon())
		{
			...
			if (pScorer->IsPlayer())
			{
				*iWeaponID = ToTFCPlayer(pScorer)->GetActiveTFCWeapon()->GetWeaponID();
			}
		}
	}
	else if (pInflictor)
	{
		...
		CWeaponTFCBase *pWeapon = dynamic_cast< CWeaponTFCBase * >(pInflictor);
		if (pWeapon)
		{
...
CTFCPlayer *CTFCGameRules::GetRecentDamager(CTFCPlayer *pVictim, int iDamager, float flMaxElapsed)
{
	...
	{
		CTFCPlayer *pRecentDamager = ToTFCPlayer(damagerHistory.hDamager);
		if (pRecentDamager)
...
//Get whom assisted
CBasePlayer *CTFCGameRules::GetAssister(CBasePlayer *pVictim, CBasePlayer *pScorer, CBaseEntity *pInflictor)
{
	CTFCPlayer *pTFCScorer = ToTFCPlayer(pScorer);
	CTFCPlayer *pTFCVictim = ToTFCPlayer(pVictim);

	if (pTFCScorer && pTFCVictim)
	{
		if (pTFCScorer == pTFCVictim)
			return NULL;

		//See whom damaged the player 2nd most, Most recent being the killer.
		CTFCPlayer *pRecentDamager = GetRecentDamager(pTFCVictim, 1, ASSIT_KILL_TIME);
...
//			*pInflictor - 
//-----------------------------------------------------------------------------
void CTFCGameRules::DeathNotice(CBasePlayer *pVictim, const CTakeDamageInfo &info)
{
	int killer_ID = 0;

	// Find the killer & the scorer
	CTFCPlayer *pTFCPlayerVictim = ToTFCPlayer(pVictim);
	CBaseEntity *pInflictor = info.GetInflictor();
	CBaseEntity *pKiller = info.GetAttacker();
	CBasePlayer *pScorer = GetDeathScorer(pKiller, pInflictor, pVictim);
	CTFCPlayer *pAssister = ToTFCPlayer(GetAssister(pVictim, pScorer, pInflictor));

	// Work out what killed the player, and send a message to all clients about it
	int iWeaponID;
	const char *killer_weapon_name = GetKillingWeaponName(info, pTFCPlayerVictim, &iWeaponID);
...
CWeaponTFCBase* IsManagedObjectAWeapon(CBaseEntity *pObject)
{
	return dynamic_cast< CWeaponTFCBase*>(pObject);
}
...
bool GetObjectsOriginalParameters(CBaseEntity *pObject, Vector &vOriginalOrigin, QAngle &vOriginalAngles)
{
	...
	else if (CWeaponTFCBase *pWeapon = IsManagedObjectAWeapon(pObject))
	{
...
void CTFCGameRules::ManageObjectRelocation(void)
{
...
//=========================================================
void CTFCGameRules::AddLevelDesignerPlacedObject(CBaseEntity *pEntity)
{
...
//=========================================================
void CTFCGameRules::RemoveLevelDesignerPlacedObject(CBaseEntity *pEntity)
{
	if (m_hRespawnableItemsAndWeapons.Find(pEntity) != -1)
...
//=========================================================
Vector CTFCGameRules::VecItemRespawnSpot(CItem *pItem)
{
...
//=========================================================
QAngle CTFCGameRules::VecItemRespawnAngles(CItem *pItem)
{
...
//=========================================================
float CTFCGameRules::FlItemRespawnTime(CItem *pItem)
{
...
//=========================================================
float CTFCGameRules::FlWeaponRespawnTime(CBaseCombatWeapon *pWeapon)
{
#ifndef CLIENT_DLL
...
// to spawn again.
//=========================================================
float CTFCGameRules::FlWeaponTryRespawn(CBaseCombatWeapon *pWeapon)
{
#ifndef CLIENT_DLL
...
// Some game variations may choose to randomize spawn locations
//=========================================================
Vector CTFCGameRules::VecWeaponRespawnSpot(CBaseCombatWeapon *pWeapon)
{
#ifndef CLIENT_DLL
	CWeaponTFCBase *pTFCWeapon = dynamic_cast< CWeaponTFCBase*>(pWeapon);
	if (pTFCWeapon)
...
//=========================================================
int CTFCGameRules::WeaponShouldRespawn(CBaseCombatWeapon *pWeapon)
{
#ifndef CLIENT_DLL
...
const char *CTFCGameRules::GetPlayerClassName(int cls, int team)
{
	CTFCTeam *pTeam = GetGlobalTFCTeam(team);
...
const char *CTFCGameRules::GetChatLocation(bool bTeamOnly, CBasePlayer* pPlayer)
{
...
const char *CTFCGameRules::GetChatFormat(bool bTeamOnly, CBasePlayer *pPlayer)
{
	if (!pPlayer)  // dedicated server output
		return NULL;


	CTFCPlayer* pTFCPlayer = ToTFCPlayer(pPlayer);
	const char *pszFormat = NULL;

	if (bTeamOnly)
	{
		if (pTFCPlayer->GetTeamNumber() == TEAM_SPECTATOR)
			pszFormat = "TFC_Chat_Spec";
		else
		{
			if (pTFCPlayer->m_lifeState != LIFE_ALIVE)
				pszFormat = "TFC_Chat_Team_Dead";
			else
				pszFormat = "TFC_Chat_Team";
		}
	}
	else
	{
		if (pTFCPlayer->GetTeamNumber() == TEAM_SPECTATOR)
			pszFormat = "TFC_Chat_All_Spec";
		else
		{
			if (pTFCPlayer->m_lifeState != LIFE_ALIVE)
				pszFormat = "TFC_Chat_All_Dead";
			else
				pszFormat = "TFC_Chat_All";
		}
	}
...
void CTFCGameRules::InitDefaultAIRelationships(void)
{
	int i, j;
...
const char* CTFCGameRules::AIClassText(int classType)
{
	switch (classType)
	{
...
//-----------------------------------------------------------------------------
int CTFCGameRules::PlayerRelationship(CBaseEntity *pPlayer, CBaseEntity *pTarget)
{
#ifndef CLIENT_DLL
...
float CTFCGameRules::GetMapRemainingTime()
{
#ifdef GAME_DLL
...
float CTFCGameRules::GetMapElapsedTime(void)
{
