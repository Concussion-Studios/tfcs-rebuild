...
class CTFCViewVectors : public CViewVectors
{
public:
	CTFCViewVectors(
		Vector vView,
...
public:
	...
	virtual const CTFCViewVectors *GetTFCViewVectors() const;
	...
	virtual const char *GetGameDescription(void) { return TFC_GAME_DESCRIPTION; }
	...
	bool CanPlayerJoinClass(CTFCPlayer *pPlayer, int cls);
	void ChooseRandomClass(CTFCPlayer *pPlayer);
...
public:
	...
	const char *GetKillingWeaponName(const CTakeDamageInfo &info, CTFCPlayer *pVictim, int *iWeaponID);
	CBasePlayer* GetAssister(CBasePlayer *pVictim, CBasePlayer *pScorer, CBaseEntity *pInflictor);
	CTFCPlayer* GetRecentDamager(CTFCPlayer *pVictim, int iDamager, float flMaxElapsed);
