...
BEGIN_RECV_TABLE_NOBASE( CTFCPlayerShared, DT_TFCPlayerShared )
	// Local Data.
	RecvPropDataTable( "tfc_sharedlocaldata", 0, 0, &REFERENCE_RECV_TABLE( DT_TFCPlayerSharedLocal ) ),
END_RECV_TABLE()
...
BEGIN_SEND_TABLE_NOBASE( CTFCPlayerShared, DT_TFCPlayerShared )
	// Local Data.
	SendPropDataTable( "tfc_sharedlocaldata", 0, &REFERENCE_SEND_TABLE( DT_TFCPlayerSharedLocal ), SendProxy_SendLocalDataTable ),	
END_SEND_TABLE()
#endif
...
bool CTFCPlayerShared::IsDucking( void ) const
{
	return ( m_pOuter->GetFlags() & FL_DUCKING ) ? true : false;
}

bool CTFCPlayerShared::IsOnGround() const
{
	return ( m_pOuter->GetFlags() & FL_ONGROUND ) ? true : false;
}

bool CTFCPlayerShared::IsOnGodMode() const
{
	return ( m_pOuter->GetFlags() & FL_GODMODE ) ? true : false;
}

int CTFCPlayerShared::GetButtons()
{
	return m_pOuter->m_nButtons;
}

bool CTFCPlayerShared::IsButtonPressing( int btn )
{
	return ( ( m_pOuter->m_nButtons & btn ) ) ? true : false;
}

bool CTFCPlayerShared::IsButtonPressed( int btn )
{
	return ( ( m_pOuter->m_afButtonPressed & btn ) ) ? true : false;
}

bool CTFCPlayerShared::IsButtonReleased( int btn )
{
	return ( ( m_pOuter->m_afButtonReleased & btn ) ) ? true : false;
}

void CTFCPlayerShared::SetJumping( bool bJumping )
{
	m_bJumping = bJumping;
...
void CTFCPlayerShared::SetGoalState( bool state )
{
	m_bInGoalZone = state;
}
...
//-----------------------------------------------------------------------------
Vector CTFCPlayerShared::GetAttackSpread( CWeaponTFCBase *pWeapon, CBaseEntity *pTarget )
{
	if (pWeapon)
...
void CTFCPlayerShared::ForceUnzoom( void )
{
	CWeaponTFCBase *pWeapon = GetActiveTFCWeapon();
	if( pWeapon && ( pWeapon->GetTFCWpnData().m_bIsSniper ) )
	{
		CWeaponTFCBaseSniper *pSniper = dynamic_cast<CWeaponTFCBaseSniper *>( pWeapon );
		if ( pSniper )
			pSniper->ExitScope();
	}
}

bool CTFCPlayerShared::IsSniperZoomed( void ) const
{
	CWeaponTFCBase *pWeapon = GetActiveTFCWeapon();
	if( pWeapon && ( pWeapon->GetTFCWpnData().m_bIsSniper ) )
	{
		CWeaponTFCBaseSniper *pSniper = (CWeaponTFCBaseSniper *)pWeapon;
		Assert( pSniper );
...
void CTFCPlayerShared::SetDesiredPlayerClass( int playerclass )
{
	m_iDesiredPlayerClass = playerclass;
}

int CTFCPlayerShared::DesiredPlayerClass( void )
{
...
int CTFCPlayerShared::PlayerClass( void )
{
...
CWeaponTFCBase* CTFCPlayerShared::GetActiveTFCWeapon() const
{
	...
	if ( pWeapon )
	{
		Assert( dynamic_cast< CWeaponTFCBase* >( pWeapon ) == static_cast< CWeaponTFCBase* >( pWeapon ) );
		return static_cast< CWeaponTFCBase* >( pWeapon );
	}
	else
	{
		return NULL;
...
void CTFCPlayerShared::ComputeWorldSpaceSurroundingBox( Vector *pVecWorldMins, Vector *pVecWorldMaxs )
{
...
// --------------------------------------------------------------------------------------------------- //
// CTFCPlayer shared implementations.
// --------------------------------------------------------------------------------------------------- //
void CTFCPlayer::FireBullet( 
...
bool CTFCPlayer::CanMove( void ) const
{
...
bool CTFCPlayer::CanAttack( void )
{
	return true;
}
...
// Output : const Vector
//-----------------------------------------------------------------------------
const Vector CTFCPlayer::GetPlayerMins( void ) const
{
	if ( IsObserver() )
...
// Output : const Vector
//-----------------------------------------------------------------------------
const Vector CTFCPlayer::GetPlayerMaxs( void ) const
{	
	if ( IsObserver() )
...
//CTFCPlayerClassInfo *CTFCPlayer::GetClassInfo(void)
//{
//	
//}

CWeaponTFCBase *CTFCPlayer::GetWeaponOwnerID( int iID )
{
...
void CTFCPlayer::InitSpeeds()
{
	...
	{
		m_Shared.m_flMaxSpeed = TFC_DEFAULT_PLAYER_RUNSPEED;
	}
	else
	{
...
//-----------------------------------------------------------------------------
bool CTFCPlayer::ShouldCollide( int collisionGroup, int contentsMask ) const
{
...
// Purpose:
//-----------------------------------------------------------------------------
void CTFCPlayer::PlayStepSound( Vector &vecOrigin, surfacedata_t *psurface, float fvol, bool force )
{
