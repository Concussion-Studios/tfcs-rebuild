...
public:
	...
	virtual void SetProgressOnStart() {	m_bOpenProgressOnStart = true; }

	// state
	bool IsInLevel();
	bool IsInBackgroundLevel();
	bool IsInMultiplayer();
	bool IsConsoleUI() { return false; }
	bool HasSavedThisMenuSession() { return m_bHasSavedThisMenuSession; } // returns true if we've saved without closing the menu
	void SetSavedThisMenuSession( bool bState )	{ m_bHasSavedThisMenuSession = bState; }
